...
func (field FieldInt64) Deserialise(inputSource *bytes.Buffer, pMap *presencemap.PresenceMap, dictionary *dictionary.Dictionary) (fix.Value, error) {
	...
	if field.Operation.ShouldReadValue(pMap) {
		...
		if err != nil {
			field.FieldDetails.Logger.Printf("[FieldInt64][%#v][%#v] failed to apply operation with readValue %#v, previousValue: %#v, reason: %s", field.FieldDetails, field.Operation, value, previousValue, err)
			return nil, fmt.Errorf("[FieldInt64][%#v][%#v] failed to apply operation with readValue %#v, previousValue: %#v, reason: %s", field.FieldDetails, field.Operation, value, previousValue, err)
		}
