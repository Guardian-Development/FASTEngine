...
func (field FieldUInt64) Deserialise(inputSource *bytes.Buffer, pMap *presencemap.PresenceMap, dictionary *dictionary.Dictionary) (fix.Value, error) {
	...
	if field.Operation.ShouldReadValue(pMap) {
		...
		if err != nil {
			field.FieldDetails.Logger.Printf("[FieldUInt64][%#v][%#v] failed to apply operation with readValue %#v, previousValue: %#v, reason: %s", field.FieldDetails, field.Operation, value, previousValue, err)
			return nil, fmt.Errorf("[FieldUInt64][%#v][%#v] failed to apply operation with readValue %#v, previousValue: %#v, reason: %s", field.FieldDetails, field.Operation, value, previousValue, err)
		}
